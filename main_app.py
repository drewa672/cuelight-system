...
class MqttWorker(QObject):
    # Signals for connection status and message reception
    connection_status = Signal(bool)
    message_received = Signal(str, str)

    def __init__(self, broker, port, topics_to_subscribe=None):
        # Initialize the MQTT worker with broker, port, and topics to subscribe
        super().__init__()
        self.client = None
        self.broker = broker
        self.port = port
    ...
    @Slot()
    def run(self):
        # Start the MQTT client loop, connect to the broker, and handle messages
    ...
    def update_subscriptions(self, topics):
        # Update the list of topics to subscribe to, unsubscribing from old topics and subscribing to new ones
        if self.client and self.client.is_connected():
            if self.topics:
...
class ChannelColumnWidget(QFrame):
    # Signal to request a status change for a channel
    status_change_requested = Signal(int, str)

    def __init__(self, numeric_id, parent=None):
        # Initialize the ChannelColumnWidget with a numeric ID, set up the UI, and create a countdown timer for "go" status
    ...
    def _init_ui(self):
        # Set up the layout and widgets for the channel column, including labels, buttons, and a list for confirmed subscribers
    ...
    def master_sb_clicked(self):
        # Toggle between "standby_master" and "idle" status
    ...
    def solo_op_clicked(self):
        #  Toggle between "standby_solo" and "go" status
    ...
    def update_display(self, data):
        # Update the display based on the provided data dictionary
        ...
        if self.current_status in ["standby_master", "standby_solo", "go"]:
            # Update the cue info label with the current cue label if available
            self.cue_info_label.setText(cue_label)
        else:
            self.cue_info_label.setText("")

        if self.current_status in ["standby_master", "standby_solo"]:
            # Set the countdown seconds based on the current status
    ...
    def _update_countdown_display(self):
        # Update the countdown display every second when in "go" status
...
class ChannelConfigWidget(QWidget):
    # Signal to request saving the configuration and changing views
    config_save_requested = Signal(dict)
    view_change_requested = Signal(str)
    def __init__(self, channels_data, parent=None):
        # Initialize the ChannelConfigWidget with channels data, set up the UI, and create input fields for channel configuration
        super().__init__(parent); self.widgets = {}; self._init_ui(); self.update_config(channels_data) 
    def update_config(self, channels_data):
        # Update the configuration with new channel data, preserving existing settings
    ...
    def _init_ui(self):
        # Set up the layout and widgets for channel configuration, including labels, input fields, and buttons
        ...
        for i in range(1, 9):
            # Create input fields for each channel, including label and color selection
    ...
    def save_changes(self):
        # Validate input and emit a signal to save the configuration
        for i, widget_group in self.widgets.items():
...
class CueEditDialog(QDialog):
    # Signal to request deletion of a cue
    def __init__(self, cue_data, all_channels_data, parent=None):
        # Initialize the CueEditDialog with cue data and all channels data, set up the UI, and create input fields and checkboxes
    ...
    def _init_ui(self):
        # Set up the dialog layout, input fields for cue number and label, and checkboxes for channels
        ...
        for i in range(1, 9):
            # Create a checkbox for each channel, pre-checking those already in the cue
    ...
    def on_save(self):
        # Validate input and save the cue data, emitting a signal to update the cue list
        ...
        self.accept()
    def on_delete(self):
        # Confirm deletion of the cue and emit a signal to delete it
...
class CueStatusDisplay(QFrame):
    # Signal to update the status of all channels
    def __init__(self, channels_data, parent=None):
        # Initialize the CueStatusDisplay with channels data, set up the UI, and create labels for each channel
    ...
    def _init_ui(self, channels_data):
        # Create a grid layout for displaying channel status labels
    ...
    def update_all(self, channels_data):
        # Update all channel labels based on the provided channels data
    ...
    def update_single(self, channel_id, data):
        # Update the label for a specific channel based on its status and other properties
...
class CueListWidget(QWidget):
    # Signal to request actions on cues, such as adding or editing
    cue_action_requested = Signal(str, object)
    def __init__(self, cues_data, channels_data, parent=None):
        # Initialize the CueListWidget with cues and channels data, set up the UI, and connect signals
        super().__init__(parent); self.channels_data = channels_data; self._init_ui(); self.update_cues(cues_data)
    def _init_ui(self):
        # Create the main layout, status display, header, and table for displaying cues
    ...
    def populate_table(self):
        # Populate the table with cues data, sorting by cue number and setting up each row with cue details and action buttons
        ...
        for row, cue in enumerate(self.cues_data):
            # Create a new row for each cue, setting the cue number, label, channels, and action button
...
class TransmitterWindow(QMainWindow):
    # Initialize the TransmitterWindow with necessary attributes and UI components
    def __init__(self):
        # Initialize the base class and set up the main window
    ...
    def _init_ui(self):
        # Create the main widget and layout, set up menus, and initialize the top navigation, header, and content stack
    ...
    def _create_menus(self):
        # Create the menu bar and add actions for file operations
    ...
    def handle_startup_choice(self):
        # Display a message box to choose between starting a new show, loading an existing show, or continuing the last session
    ...
    def create_top_nav(self):
        # Create the top navigation bar with buttons for different modes and channel configuration
    ...
    def create_header(self):
        # Create the header widget with MQTT status, controls for master go, and cue controls
    ...
    def create_cue_controls(self):
        # Create the cue controls widget with buttons for navigating and controlling cues
        ...
        return widget
    def setup_mqtt(self):
        # Set up the MQTT worker thread and connect it to the confirmation topic
    ...
    def on_confirmation_received(self, topic, payload):
        # Handle incoming MQTT confirmation messages, updating the channel data and confirmed subscribers
        try:
    ...
    def load_config(self, filepath=None):
        ...
        if os.path.exists(target_file):
            # Attempt to read the configuration file and update the channels and cues
            try:
    ...
    def create_default_config(self):
        # Create a default configuration with 8 channels, each with a unique color
        self.channels_data = {};
        for i in range(1, 9):
    ...
    def save_config(self, filepath=None):
        # Save the current configuration to a file, or prompt for a file name if none is provided
    ...
    def handle_save_config_as(self):
        # Prompt the user to choose a file name for saving the configuration
    ...
    def handle_load_config(self):
        # Prompt the user to choose a file to load the configuration from
    ...
    def _update_nav_buttons(self, active_button):
        # Update the navigation buttons to reflect the active view
    ...
    def on_config_saved(self, new_channels_data):
        # Update the channels data with the new configuration and publish updates to MQTT
    ...
    def create_manual_view(self):
        # Create the manual control view with channel widgets for manual operation
    ...
    def update_all_channel_displays(self):
        # Update all channel widgets with the current channels data
    ...
    def handle_status_change(self, numeric_id, new_status):
        # Handle status changes for a channel, updating its data and publishing to MQTT
    ...
    def revert_go_to_idle(self, numeric_id):
        # Revert the channel status from "go" to "idle" after a specified duration
        numeric_id_str = str(numeric_id)
    ...
    def fire_master_go(self):
        # Handle the master go button press, setting all channels to "standby_master" and updating their displays
    ...
    def update_cue_header_display(self):
        # Update the cue header display based on the current cue index and whether it is armed
    ...
    def arm_current_cue(self):
        # Arm the current cue, setting all channels in the cue to "standby_master" status
    ...
    def go_current_cue(self):
        # Execute the current cue, setting all channels in the cue to "go" status and moving to the next cue
    ...
    @Slot()
    def next_cue(self):
        # Move to the next cue in the list, wrapping around if necessary
    ...
    @Slot()
    def prev_cue(self):
        # Move to the previous cue in the list, wrapping around if necessary
        if len(self.cues) == 0: return
    ...
    def handle_cue_action(self, action, data):
        # Handle actions for cues, such as adding, editing, or deleting cues
        if action == "add":
    ...
    def closeEvent(self, event):
        # Handle the close event, saving the configuration and stopping the MQTT worker
...
class ReceiverSettingsDialog(QDialog):
    # This dialog allows the user to configure receiver settings such as name, channel, and broker IP.
...
class ReceiverWindow(QMainWindow):
    # This class represents the main window for the receiver application, handling MQTT connections and UI interactions.
    def __init__(self):
    ...
    def load_settings(self):
        # Load receiver settings from file or create default values
    ...
    def save_settings(self, name, channel_id, broker_ip):
        # Save receiver settings to file
    ...
    def _get_or_create_receiver_id(self):
        # Load or create a unique receiver ID
        try:
            ...
            return new_id
    def setup_mqtt(self):
        # Set up MQTT connection and subscribe to topics
    ...
    def handle_mqtt_message(self, topic, payload):
        # Handle incoming MQTT messages and update the display accordingly
    ...
    def update_background_and_text(self, bg_color_hex="#E0E0E0", text_color_hex="#000000"):
        # Update the background and text colors of the main window
    ...
    def update_connection_status(self, connected):
        # Update the connection status and UI based on MQTT connection state
        if not connected: self.update_background_and_text("#505050"); self.status_label.setText("DISCONNECTED")
    def open_settings_dialog(self):
        # Open the settings dialog to change receiver settings
...
# =============================================================================
def get_device_role():
    # Checks for a role configuration file to determine if the device is a transmitter or receiver.
...
if __name__ == "__main__":
    ...
    # Select your operating mode.  If not explicitly set, defaults to receiver.
    if role == "transmitter": 
        window = TransmitterWindow()
    else:
        window = ReceiverWindow()
    
    # Check for command line arguments to set fullscreen mode
    window.show()
